...
ivdx::dx12::core::core( HWND hWnd )
{
  ...
  // Create D3D device
  D3D12CreateDevice(BestAdapter, D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(Device.GetAddressOf()));
  BestAdapter->Release();
  ...
  FLT aspectRatio = (FLT)Width / (FLT)Height;
  ...
  Device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &buffer, D3D12_RESOURCE_STATE_GENERIC_READ,
  ...
  ReadRange.Begin = 0;
  ReadRange.End = 0;
  VBuffer->Map(0, &ReadRange, reinterpret_cast<void**>(&pVertexDataBegin));
  memcpy(pVertexDataBegin, triangleVertices, sizeof(triangleVertices));
  VBuffer->Unmap(0, nullptr);

  // Initialize the vertex buffer view.
  VertexBufferView.BufferLocation = VBuffer->GetGPUVirtualAddress();
